...
import { convertToWebp, convertToJpeg, convertToPng, convertToAvif } from '@/services/drive/image-processor.js';
				...
				const convertFile = async () => {
					if (isThumbnail) {
						if (['image/jpeg', 'image/webp', 'image/png', 'image/svg+xml', 'image/avif'].includes(mime)) {
							// Try AVIF first for better compression, fallback to WebP
							try {
								return await convertToAvif(path, 498, 280);
							} catch {
