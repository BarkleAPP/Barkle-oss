...
router.get('/stream/@:acct', async ctx => {
	const { username, host } = Acct.parse(ctx.params.acct);
	const user = await Users.findOne({
		where: {
			usernameLower: username.toLowerCase(),
	...
	// Convert Mux URL to m3u8 URL
	// From: https://stream.mux.com/{PLAYBACK_ID}
	// To: https://stream.mux.com/{PLAYBACK_ID}.m3u8
	const m3u8Url = `${stream.url}.m3u8`;
