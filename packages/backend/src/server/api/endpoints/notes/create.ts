...
import { signalCollectionService, SignalContext } from '@/services/algorithm/signal-collection-service.js';
...
export const meta = {
	...
	errors: {
		...
		bannedFromChannel: {
			message: 'You are banned from posting in this channel.',
			code: 'BANNED_FROM_CHANNEL',
			id: 'fe8d7103-0ea8-4d3c-b17e-f685e5459d51',
		},

		channelArchived: {
			message: 'This channel has been archived. You cannot post in archived channels.',
			code: 'CHANNEL_ARCHIVED',
			id: 'c4d9f88e-9270-4632-b032-a75d25a32636',
		},

		alreadyRenoted: {
			message: 'You have already renoted this note.',
			code: 'ALREADY_RENOTED',
			id: '3c6a57e8-d8a1-4457-9ad7-c1f9c0b34e48',
		},
	},
} as const;
...
export const paramDef = {
	...
	properties: {
		...
		barkleFor: { type: 'string', maxLength: 256, nullable: true },
		...
		fileIds: {
			...
			minItems: 1,
			maxItems: 16,
			items: { type: 'string', format: 'barkle:id' },
		},
		mediaIds: {
			deprecated: true,
		...
		replyId: { type: 'string', format: 'barkle:id', nullable: true },
		renoteId: { type: 'string', format: 'barkle:id', nullable: true },
		channelId: { type: 'string', format: 'barkle:id', nullable: true },
		poll: {
			type: 'object',
			nullable: true,
			properties: {
...
export default define(meta, paramDef, async (ps, user) => {
	...
	if (ps.renoteId != null) {
		...
		// Check if this is a pure renote and if the user has already renoted this note
		if (!ps.text && !fileIds && !ps.poll) {
			...
			if (existingRenote) {
				throw new ApiError(meta.errors.alreadyRenoted);
			}
		}
	}
	...
	if (ps.channelId != null) {
		...
		if (channel.archive) {
			throw new ApiError(meta.errors.channelArchived);
		}
	...
	const note = await create(user, {
		...
		barkleFor: ps.barkleFor || undefined,
		reply,
		renote,
		cw: ps.cw,
	...
	try {
		// Categorize the note
		contentCategorizationService.categorizeNote(note);

		// Collect signals
		const context: SignalContext = { source: 'direct' };
		if (note.replyId) {
			signalCollectionService.collectSignal(user.id, 'note_reply', note.replyId, context);
		}
		if (note.renoteId) {
			// Treat renotes as shares for signal collection
			signalCollectionService.collectSignal(user.id, 'note_share', note.renoteId, context);
		}
	} catch (error) {
		// Do not block note creation if algorithm services fail
		console.error('Failed to collect algorithm data for new note:', error);
	}
