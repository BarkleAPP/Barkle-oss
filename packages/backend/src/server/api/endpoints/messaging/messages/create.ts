...
export const paramDef = {
	...
	properties: {
		...
		replyId: { type: 'string', format: 'barkle:id' },
		shouldEncrypt: { type: 'boolean', default: true },
		userId: { type: 'string', format: 'barkle:id' },
		groupId: { type: 'string', format: 'barkle:id' },
	},
	anyOf: [
		{
			properties: {
...
export default define(meta, paramDef, async (ps, user) => {
	...
	return await createMessage(
		user, 
		recipientUser ? { id: recipientUser.id } : undefined, 
		recipientGroup || undefined, 
		ps.text, 
		file, 
		ps.replyId || null,
		ps.shouldEncrypt || false
	);
});
