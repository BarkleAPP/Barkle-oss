...
export default define(meta, paramDef, async (ps, user) => {
	...
	if (recipientId) {
		publishMessagingStream(recipientId, user.id, userReacted ? 'reaction' : 'reactionRemoved', {
			messageId: ps.messageId,
			reaction: ps.reaction,
			userId: user.id,
			reactionCounts: currentCounts,
	...
	if (message.groupId) {
		publishGroupMessagingStream(message.groupId, userReacted ? 'reaction' : 'reactionRemoved', {
			messageId: ps.messageId,
			reaction: ps.reaction,
			userId: user.id,
			reactionCounts: currentCounts,
