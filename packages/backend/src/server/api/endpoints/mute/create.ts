...
export default define(meta, paramDef, async (ps, user) => {
	...
	try {
		signalCollectionService.collectSignal(muter.id, 'mute', mutee.id, { source: 'direct' });
	} catch (error) {
		// Do not block mute action if algorithm service fails
		console.error('Failed to collect mute signal:', error);
	}
