...
export const meta = {
  tags: ['gift'],
  requireCredential: false, // Allow non-logged in users to verify gifts before redeeming
  limit: {
    duration: HOUR,
    max: 30, // Allow more verification attempts than redemption
  },
  errors: {
    INVALID_TOKEN: {
...
export default define(meta, paramDef, async (ps, user) => {
  // Use our enhanced repository method to find the gift regardless of status
  const gift = await GiftedSubscriptions.findGiftByToken(ps.token);
