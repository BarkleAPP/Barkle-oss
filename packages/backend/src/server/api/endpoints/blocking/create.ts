...
export default define(meta, paramDef, async (ps, user) => {
	...
	// Collect signal for algorithm
	try {
		signalCollectionService.collectSignal(blocker.id, 'block', blockee.id, { source: 'direct' });
	} catch (error) {
		// Do not block block action if algorithm service fails
		console.error('Failed to collect block signal:', error);
	}
