...
export default async (ctx: Koa.Context) => {
	...
	if (instance.preReleaseMode) {
		const isAllowedRole = instance.preReleaseAllowedRoles.some(role => {
			switch (role) {
				...
				case 'plus': return user.isPlus;
				case 'mplus': return user.isMPlus;
				default: return false;
			}
		});
		
		const isAllowedUser = instance.preReleaseAllowedUserIds.includes(user.id);
		
		if (!isAllowedRole && !isAllowedUser) {
			error(403, {
				id: 'pre-release-access-denied',
			});
			ctx.body = {
				error: {
					message: 'Access is currently restricted to select users during our pre-release phase.',
					code: 'PRE_RELEASE_ACCESS_DENIED',
					id: 'pre-release-access-denied'
				}
			};
			return;
