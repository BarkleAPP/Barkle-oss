...
export interface InternalStreamTypes {
	...
	userChangeisVerifiedState: { id: User['id']; isVerified: User['isVerified']; };
	userPlusOveride: { id: User['id']; isPlus: User['isPlus']; };
	userMPlusOveride: { id: User['id']; isMPlus: User['isMPlus']; };
	userChangeStaffState: { id: User['id']; isStaff: User['isStaff']; };
	userChangeTranslatorState: { id: User['id']; isTranslator: User['isTranslator']; };
...
export interface BroadcastTypes {
	decorationsAdded: {
		decorations: Packed<'Decorations'>;
	};
}
...
export interface LiveChatStreamTypes {
	message: Packed<'LiveChatMessage'>;
	deleted: {
		messageId: string;
		deletedBy: User['id'];
	};
...
export interface StreamViewersTypes {
	viewerUpdate: {
		views: number;
		viewers: number;
		updated_at: string;
...
export type NowPlayingStreamTypes = {
    song: NowPlayingSong | null; // null when nothing is playing
};
//#endregion
...
export type StreamMessages = {
	...
	liveChat: {
		name: `liveChatStream:${Streams['id']}`;
		payload: EventUnionFromDictionary<LiveChatStreamTypes>;
	};
	streamViewers: {
		name: `streamViewers:${Streams['id']}`;
		payload: EventUnionFromDictionary<StreamViewersTypes>;
	};
	notes: {
		name: 'notesStream';
		payload: Note;
	...
	nowPlaying: {
		name: `nowPlayingStream:${User['id']}`;
		payload: EventUnionFromDictionary<NowPlayingStreamTypes>;
	};
};
