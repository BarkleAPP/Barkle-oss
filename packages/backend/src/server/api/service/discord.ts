...
router.get('/dc/cb', async ctx => {
	...
	if (!userToken) {
		...
		if (profile == null) {
			ctx.throw(404, `There was no Barkle accounts linked to @${username}`);
			return;
		}
		...
		ctx.body = `Discord: @${username}#${discriminator} を、barkle: @${user.username} に接続しました！`;
