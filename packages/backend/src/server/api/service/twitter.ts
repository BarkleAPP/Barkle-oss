...
router.get('/tw/cb', async ctx => {
	...
	if (userToken == null) {
		...
		if (link == null) {
			ctx.throw(404, `The there was no Barkle account linked to @${result.screenName}`);
			return;
		}
		...
		ctx.body = `Twitter: @${result.screenName} を、barkle: @${user.username} に接続しました！`;
