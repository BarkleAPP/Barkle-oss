...
import { processFirebaseNotifications } from './processors/firebase-notifications.js';
import { queueLogger } from './logger.js';
import { getJobInfo } from './get-job-info.js';
import { systemQueue, dbQueue, deliverQueue, inboxQueue, objectStorageQueue, endedPollNotificationQueue, firebaseNotificationQueue, webhookDeliverQueue, subscriptionQueue } from './queues.js';
import { ThinUser } from './types.js';
import { processSubscription } from './processors/subscription.js';
...
const subscriptionLogger = queueLogger.createSubLogger('subscription');
...
subscriptionQueue
	.on('waiting', (jobId) => subscriptionLogger.debug(`waiting id=${jobId}`))
	.on('active', (job) => subscriptionLogger.debug(`active ${getJobInfo(job, true)} userId=${job.data.userId} action=${job.data.action}`))
	.on('completed', (job, result) => subscriptionLogger.debug(`completed(${result}) ${getJobInfo(job, true)} userId=${job.data.userId} action=${job.data.action}`))
	.on('failed', (job, err) => subscriptionLogger.warn(`failed(${err}) ${getJobInfo(job)} userId=${job.data.userId} action=${job.data.action}`))
	.on('error', (job: any, err: Error) => subscriptionLogger.error(`error ${err}`, { job, e: renderError(err) }))
	.on('stalled', (job) => subscriptionLogger.warn(`stalled ${getJobInfo(job)} userId=${job.data.userId} action=${job.data.action}`));
...
export function createExportUserDataJob(user: ThinUser, includePrivateData = true, format: 'json' | 'csv' = 'json') {
	return dbQueue.add('exportUserData', {
		user: user,
		includePrivateData,
		format,
	}, {
...
export default function() {
	...
	firebaseNotificationQueue.process(processFirebaseNotifications);
	webhookDeliverQueue.process(64, processWebhookDeliver);
	subscriptionQueue.process(processSubscription);
	processDb(dbQueue);
	...
	// Check for subscription expirations every hour
	systemQueue.add('checkSubscriptions', {
	}, {
		repeat: { cron: '0 * * * *' },
		removeOnComplete: true,
	});

	// Process Firebase notifications every 15 minutes
	firebaseNotificationQueue.add('processNotifications', {}, {
		repeat: { cron: '*/15 * * * *' },
		removeOnComplete: true,
	});
