...
import { convertToMp4, shouldTranscodeVideo } from './video-processor.js';
...
type AddFileArgs = {
	...
	/** If set to true, bypasses drive capacity limits (for system uploads like stream VODs) */
	bypassDriveCapacityCheck?: boolean;
...
export async function addFile({
	...
	uri = null,
	sensitive = null,
	bypassDriveCapacityCheck = false,
	requestIp = null,
	requestHeaders = null,
	...
	//#region Transcode video if needed
	let transcodedPath = path;
	...
	if (!isLink && shouldTranscodeVideo(info.type.mime)) {
		try {
			...
			await new Promise<void>((resolve, reject) => {
				...
				transcoded.data.pipe(writeStream);
	...
	// detect name
	const detectedName = transcodedName || (transcodedInfo.type.ext ? `untitled.${transcodedInfo.type.ext}` : 'untitled');

	if (user && !force) {
		// Check if there is a file with the same hash across all users
		const much = await DriveFiles.findOneBy({
			md5: transcodedInfo.md5,
			userId: user.id,
		});

		if (much) {
			// If a file with the same hash is found, create a new DriveFile entry
			// for the current user that points to the existing file data,
			// instead of saving a new file to the storage.
			logger.info(`file with same hash is found: ${much.id}. Creating a new reference for user ${user.id}`);

			const folder = folderId ? await DriveFolders.findOneBy({ id: folderId, userId: user.id }) : null;
			if (folderId && !folder) {
				throw new IdentifiableError('d74d7d5e-6f9e-4e10-a58d-84d24e8b9a68', 'Folder not found');
			}
			...
			file.folderId = folder ? folder.id : null;
			file.comment = comment;
			file.name = detectedName;
			file.type = much.type;
			file.size = much.size;
			file.md5 = much.md5;
			file.blurhash = much.blurhash;
			file.properties = much.properties;
			file.isLink = isLink;
			file.url = much.url;
			file.thumbnailUrl = much.thumbnailUrl;
			file.webpublicUrl = much.webpublicUrl;
			file.accessKey = much.accessKey;
			file.thumbnailAccessKey = much.thumbnailAccessKey;
			file.webpublicAccessKey = much.webpublicAccessKey;
			file.webpublicType = much.webpublicType;
			file.storedInternal = much.storedInternal;

			// Set sensitivity flags based on the current user's settings and the latest ML analysis, not the original file's flags.
			...
			const savedFile = await DriveFiles.save(file);

			logger.succ(`drive file has been created (reused) ${savedFile.id}`);

			DriveFiles.pack(savedFile, { self: true }).then(packedFile => {
			...
			// 統計を更新
			driveChart.update(savedFile, true);
			perUserDriveChart.update(savedFile, true);
			if (savedFile.userHost !== null) {
				instanceChart.updateDrive(savedFile, true);
			}
	...
	//#region Check drive capacity limit
	if (user && !isLink && !bypassDriveCapacityCheck) {
	...
	if (transcodedInfo.width) {
		properties['width'] = transcodedInfo.width;
		properties['height'] = transcodedInfo.height;
	}
	if (transcodedInfo.orientation != null) {
		properties['orientation'] = transcodedInfo.orientation;
	}
	...
	file.blurhash = transcodedInfo.blurhash || null;
	file.isLink = isLink;
	file.requestIp = requestIp;
	file.requestHeaders = requestHeaders;
	file.maybeSensitive = transcodedInfo.sensitive;
	...
	if (transcodedInfo.sensitive && instance.setSensitiveFlagAutomatically) file.isSensitive = true;
	...
	if (isLink) {
		try {
			file.size = 0;
			file.md5 = transcodedInfo.md5;
			file.name = detectedName;
			file.type = transcodedInfo.type.mime;
			file.storedInternal = false;
		...
		file = await (save(file, transcodedPath, detectedName, transcodedType, transcodedInfo.md5, transcodedInfo.size));
	}
