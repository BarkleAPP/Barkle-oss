...
export async function pushNotification<T extends keyof pushNotificationsTypes>(userId: string, type: T, body: pushNotificationsTypes[T]) {
	// Send via Firebase Cloud Messaging (new method)
	if (type === 'notification' && body) {
		try {
			await firebaseMessaging.sendBarkleNotification(userId, body as Packed<'Notification'>);
		} catch (error) {
			console.error('Firebase notification failed:', error);
		}
	}

	// Continue with existing service worker method for backwards compatibility
