...
export async function createNotification(
	...
	setTimeout(async () => {
        ...
        if (type === 'mention') sendEmailNotification.mention(notifieeId, await Users.findOneByOrFail({ id: data.notifierId! }), data.note!);
        if (type === 'reply') sendEmailNotification.reply(notifieeId, await Users.findOneByOrFail({ id: data.notifierId! }), data.note!);
        if (type === 'quote') sendEmailNotification.quote(notifieeId, await Users.findOneByOrFail({ id: data.notifierId! }), data.note!);
        //if (type === 'groupInvited') sendEmailNotification.groupInvite(notifieeId, await Users.findOneByOrFail({ id: data.notifierId! }), data.userGroup!);
