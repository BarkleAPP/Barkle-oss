...
async function getCpuFlags(): Promise<string[]> {
	try {
		...
		console.error('Error getting CPU flags:', error instanceof Error ? error.message : String(error));
		// Return empty array if we can't detect CPU flags
		// This will cause the CPU support check to fail gracefully
		return [];
	}
}
