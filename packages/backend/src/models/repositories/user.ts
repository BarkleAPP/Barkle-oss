...
export const UserRepository = db.getRepository(User).extend({
	...
	async getHasUnreadMessagingMessage(userId: User['id']): Promise<boolean> {
		...
		const groupQs = Promise.all(joinings.map(j => MessagingMessages.createQueryBuilder('message')
			...
			.andWhere('message.createdAt > :joinedAt', { joinedAt: j.createdAt })
	...
	async pack<ExpectsMe extends boolean | null = null, D extends boolean = false>(
		...
		const userNumber = await this.count({
			where: {
				createdAt: LessThan(user.createdAt),
			},
		}) + 1;
		...
		const packed = {
			...
			isStaff: user.isStaff || falsy,
			isTranslator: user.isTranslator || falsy,
			isOG: user.isOG || falsy,
			isPlus: user.isPlus || user.isStaff || falsy,
			...
			avatarDecorations: await Promise.all(user.avatarDecorations.map(async decoration => {
				const dec = await Decorations.findOne({ where: { id: decoration.id } });
				return {
				  ...decoration,
				  url: dec ? dec.publicUrl : null,
				};
			  })),
			...
			...(opts.detail ? {
				...
				securityKeys: profile!.twoFactorEnabled
					...
					: false,
				profileCss: user.isPlus ? profile!.profileCss : undefined,
				//avatarDecorations: profile.avatarDecorations,
			} : {}),
			...
			...(opts.detail && isMe ? {
				...
				lastfmUsername: UserMusicIntegrations.findOne({
					where: { userId: user.id, service: 'lastfm' },
					select: { username: true },
				}).then(result => result?.username || null),
			...
			...(opts.includeSecrets ? {
				...
				securityKeysList: profile!.twoFactorEnabled
					...
					: [],
				musicIntegrations: UserMusicIntegrations.find({
					where: {
						userId: user.id,
					},
					select: {
