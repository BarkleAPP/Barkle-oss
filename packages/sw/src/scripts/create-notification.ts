...
import type { pushNotificationDataMap, CustomNotificationOptions } from '@/types';
import type { I18n } from '@/scripts/i18n';
...
async function composeNotification<K extends keyof pushNotificationDataMap>(data: pushNotificationDataMap[K]): Promise<[string, CustomNotificationOptions] | null> {
	...
	switch (data.type) {
		...
		*/
		case 'notification':
			if (!data.body || !('type' in data.body)) return null;
			switch (data.body.type) {
				case 'follow':
					account = await getAccountFromId(data.userId);
					if (!account) return null;
					userDetail = await cli.request('users/show', { userId: data.body.userId }, account.token);
				...
				case 'reaction':
					...
					if (reaction.startsWith(':')) {
						// カスタム絵文字の場合
						customEmoji = data.body.note.emojis.find(x => x.name === reaction.substr(1, reaction.length - 2));
						if (customEmoji) {
							if (reaction.includes('@')) {
								reaction = `:${reaction.substr(1, reaction.indexOf('@') - 1)}:`;
							}

							u = new URL(customEmoji.url);
		...
		case 'unreadMessagingMessage':
			...
			if ('groupId' in data.body && data.body.groupId === null && 'user' in data.body) {
			...
			if ('group' in data.body && data.body.group && 'user' in data.body) {
