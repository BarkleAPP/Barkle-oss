...
export function getNoteMenu(props: {
	...
	function copyLink(): void {
		copyToClipboard(`${url}/barks/${appearNote.id}`);
		os.success();
	}
	...
	async function share(): Promise<void> {
		if (shareAvailable()) {
			os.popupMenu([
				...
				{
					text: i18n.ts.shareAsMessage,
					icon: 'ph-chat-circle-bold ph-lg',
					action: shareDm,
				},
			])
		} else {
	...
	function shareNet(): void {
		navigator.share({
			title: i18n.t('noteOf', { user: appearNote.user.name }),
			text: appearNote.text,
			url: `${url}/barks/${appearNote.id}`,
		});
	}

	function shareDm(): void {
		os.selectUser().then((user) => {
			os.apiWithDialog("messaging/messages/create", {
				userId: user.id,
				text: `${url}/barks/${appearNote.id}`
			});
		},'closed');
	}
	...
	if ($i) {
		...
		menu = [
			...
			{
				icon: shareAvailable() ? 'ph-share-network-bold ph-lg' : 'ph-chat-circle-bold ph-lg',//'ph-share-network-bold ph-lg',
				text: shareAvailable() ? i18n.ts.share : i18n.ts.shareAsMessage,
				action: share,
			},
		...
		...($i.isModerator || $i.isAdmin ? [
			...
			...(appearNote.userId !== $i.id ? [
				null,
				{
					...
					action: () => {
						const u = appearNote.url || appearNote.uri || `${url}/barks/${appearNote.id}`;
