...
(async () => {
	...
	const lang = await setLanguage(localStorage.getItem("lang"));
	...
	if (loginId) {
		...
		history.replaceState({ barkle: 'loginId' }, '', target);
	}
	...
	const app = createApp(
		...
		!$i ? defineAsyncComponent(() => import('@/ui/guest.vue')) :
	...
	const rootEl = (() => {
		const MISSKEY_MOUNT_DIV_ID = 'barkle_app';
