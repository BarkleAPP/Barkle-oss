	...
	<div class="_formRoot">
		<FormSection>
			...
			<FormInput v-model="emailAddress" type="email" manual-save>
				...
				<template v-else-if="emailAddress === $i.email && $i.emailVerified" #caption><i
						class="ph-check-bold ph-lg" style="color: var(--success);"></i> {{ i18n.ts.emailVerified
						}}</template>
			</FormInput>
		</FormSection>

		<FormSection>
			<FormSwitch :model-value="$i.receiveAnnouncementEmail"
		...
		<FormSection>
			...
			<FormSwitch v-model="emailNotification_socialReminder" class="_formBlock">
				{{ i18n.ts.socialReminders }}
				<template #caption>{{ i18n.ts.socialRemindersDescription }}</template>
			</FormSwitch>
		</FormSection>
	</div>
</template>
...
const emailNotification_socialReminder = ref($i!.emailNotificationTypes.includes('socialReminder'));
...
const saveNotificationSettings = () => {
	os.api('i/update', {
		emailNotificationTypes: [
			...
			...[emailNotification_socialReminder.value ? 'socialReminder' : null],
		].filter(x => x != null),
	});
};

watch([emailNotification_mention, emailNotification_reply, emailNotification_quote, emailNotification_follow, emailNotification_receiveFollowRequest, emailNotification_groupInvited, emailNotification_socialReminder], () => {
