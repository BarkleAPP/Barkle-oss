<template>
  <div class="_formRoot">
    ...
    <div class="llvierxe" :style="{ backgroundImage: $i.bannerUrl ? `url(${ $i.bannerUrl })` : null }">
      <div class="avatarContainer">
        ...
        <div class="avatarButtons">
          ...
          <MkButton primary rounded class="avatarDeco" link to="/settings/decorations">{{ i18n.ts.avatarDecorations }}</MkButton>
        </div>
      </div>
    ...
    <FormTextarea v-if="$i.isPlus" v-model="profile.profileCss" :max="2000" tall manual-save class="_formBlock">
      <template #label>{{ i18n.ts.customCss }}</template>
      <template #caption>{{ i18n.ts.customCssCaption }}</template>
    </FormTextarea>
...
const profile = reactive({
  ...
  birthday: $i?.birthday,
  lang: $i?.lang,
  lastfmUsername: $i?.lastfmUsername,
  isBot: $i?.isBot,
  isCat: $i?.isCat,
...
function save() {
  os.apiWithDialog('i/update', {
    ...
    profileCss: $i.isPlus ? profile.profileCss : undefined,
  });
}
...
.llvierxe {
  ...
  overflow: clip;
  padding: 16px;
  min-height: 120px; // Ensure enough space for avatar and buttons
