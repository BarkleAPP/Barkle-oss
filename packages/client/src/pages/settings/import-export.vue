...
<div class="_formRoot">
	<FormSection>
		<template #label>{{ i18n.ts._exportOrImport.allData }}</template>
		<template #icon><i class="ph-package-bold ph-lg"></i></template>
		<FormFolder>
			<template #label>{{ i18n.ts.export }}</template>
			<template #icon><i class="ph-download-simple-bold ph-lg"></i></template>
			<div class="_formBlock">
				<FormSwitch v-model="includePrivateData" class="_formBlock">
					{{ i18n.ts._exportOrImport.includePrivateData }}
				</FormSwitch>
				<p style="color: var(--fg); margin-top: 8px; font-size: 0.85em;">
					{{ i18n.ts._exportOrImport.allDataDescription }}
				</p>
			</div>
			<MkButton primary :class="$style.button" inline @click="exportAllData()">
				<i class="ph-download-simple-bold ph-lg"></i> {{ i18n.ts.export }}
			</MkButton>
		</FormFolder>
	</FormSection>
	<FormSection>
...
const includePrivateData = ref(true);
...
const exportAllData = () => {
	os.api('i/export-user-data', {
		includePrivateData: includePrivateData.value,
		format: 'json'
	}).then(() => {
		os.alert({
			type: 'info',
