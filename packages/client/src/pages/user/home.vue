<template>
	<MkSpacer :content-max="narrow ? 800 : 1100">
		<div ref="rootEl" v-size="{ max: [500] }" class="ftskorzw" :class="{ wide: !narrow }">
			<div class="main">
				<div v-if="user.isSuspended" class="punished"><i class="ph-warning-bold ph-lg" style="margin-right: 8px;"></i> {{ i18n.ts.userSuspended }}</div>
				<div v-else-if="user.isSilenced" class="punished"><i class="ph-warning-bold ph-lg" style="margin-right: 8px;"></i> {{ i18n.ts.userSilenced }}</div>
				<div v-if="user.isBlocked" class="punished"><i class="ph-warning-bold ph-lg" style="margin-right: 8px;"></i> {{ i18n.ts.youreBlocked }}</div>
				<div v-else-if="user.isBlocking" class="punished"><i class="ph-warning-bold ph-lg" style="margin-right: 8px;"></i> {{ i18n.ts.blockedUser }}</div>
				<div class="profile">
					<div :key="user.id" class="_block main">
						<div class="banner-container" :style="style">
							...
							<div class="title">
								<div class="nameCollumn">
									...
									<!-- <span v-if="user.isPlus" :title="i18n.ts.isPlus"><i class="ph-dog-bold"></i></span> -->
						...
						<!-- <div class="fields system">
							<div class="spotifyNP">
								<div class="spotify-logo">
									<i class="ph ph-spotify-logo" style="display: flex; font-size: 20px;"></i>
								</div>
								<div class="SNPContent">
									<img class="SNPCover" src="https://i.pinimg.com/originals/2f/63/06/2f63062775249cbd4639788eeb7d66d9.jpg"/>
									<div class="SNPTextWrapper">
										<span class="SNPText" style="font-size: 110%; font-weight: bold;">In the Meantime</span>
										<span class="SNPText">Spacehog</span>
									</div>
								</div>
							</div>
						...
						<div class="fields system">
							...
							<dl v-if="user.userNumber <= 300" class="early-joiner-badge field" :title="i18n.ts.earlyJoiner">
							    <dt class="name"><i class="ph-flower-lotus-bold ph-lg ph-fw ph-lg"></i> {{ i18n.ts.earlyUser }}</dt>
							    <dd class="value">#{{ user.userNumber }}</dd>
						    </dl>
						</div>
...
import type { NowPlayingSong } from '@/components/NowPlaying.vue';
...
import { stream } from '@/stream';
	...
	const nowPlayingSong = ref<NowPlayingSong | null>(null);
	...
	function requestImmediateNowPlaying() {
		console.log('Requesting immediate now playing data for user:', props.user.id);
		os.api('music/now-playing', {
			userId: props.user.id
		}).then((data: any) => {
			console.log('Immediate now playing response:', data);
			if (data && data.isPlaying) {
				nowPlayingSong.value = data;
			} else {
				nowPlayingSong.value = null;
			}
		}).catch((error: any) => {
			console.log('Failed to get immediate now playing data:', error);
		});
	}
	...
	onMounted(() => {
		...
		// Connect to now playing stream for this user
		console.log('Setting up now playing stream for user:', props.user.id);
		
		// Use the stream API to connect to the nowPlayingStream channel
		const connectionId = Math.random().toString(36).substring(7);
		
		// Hook into the actual WebSocket to intercept messages
		const ws = (stream as any).stream;
		...
		const handleWebSocketMessage = (event: MessageEvent) => {
			try {
				...
				// Log all channel messages to see what we're getting
				if (data.type === 'channel') {
					console.log('Channel message received:', data);
				}
				
				// Log connected messages
				if (data.type === 'connected') {
					console.log('Connected message:', data);
				}
				
				// Check if this is a channel message for our nowPlayingStream connection
				if (data.type === 'channel' && data.body && data.body.id === connectionId) {
					console.log('Channel message for nowPlayingStream:', data.body);
					if (data.body.type === 'song') {
						console.log('Setting now playing song:', data.body.body);
						// Only set if it's actively playing, otherwise clear it
						if (data.body.body && data.body.body.isPlaying) {
							nowPlayingSong.value = data.body.body;
						} else {
							nowPlayingSong.value = null;
						}
					}
		...
		if (ws && ws.addEventListener) {
			...
			// Fallback for older WebSocket API
		...
		stream.send('connect', {
			channel: 'nowPlayingStream',
			id: connectionId,
			params: { userId: props.user.id }
		});
		
		console.log('Sent nowPlayingStream connection request with ID:', connectionId);
		
		// Request immediate now playing data
		requestImmediateNowPlaying();
		
		connection = {
			id: connectionId,
			dispose: () => {
				stream.send('disconnect', { id: connectionId });
				if (ws && ws.removeEventListener) {
					ws.removeEventListener('message', handleWebSocketMessage);
				} else if (ws && originalOnMessage) {
	...
	.spotifyNP {
		...
		background: rgb(34, 158, 130);
		border: solid 1px rgb(27, 104, 86);
		padding: 10px;
		padding-bottom: 20px;
	}
	.SNPContent {
