...
<div v-size="{ max: [400, 500] }" class="thvuemwp" :class="{ isMe }">
	...
	<div class="content">
		...
		<div v-for="embeddedNote in embeddedNotes" :key="embeddedNote.id" class="embedded-note">
			<XNote :note="embeddedNote"/>
		</div>
...
import XNote from '@/components/MkNoteMessage.vue';
...
function scrollToBottom() {
  ...
  if (container) {
    const scrollContainer = container.closest('.body');
    if (scrollContainer) {
      nextTick(() => {
...
function extractNoteUrls(text) {
  const regex = /https?:\/\/[^\s/]+\/notes\/([a-zA-Z0-9]+)/g;
  const matches = text.match(regex);
  return matches ? matches : [];
}

async function fetchEmbeddedNotes() {
  if (!props.message.text) return;
  
  const noteUrls = extractNoteUrls(props.message.text);
  
  for (const url of noteUrls) {
    const noteId = url.split('/').pop();
    try {
      const note = await os.api('notes/show', { noteId });
      embeddedNotes.value.push(note);
    } catch (error) {
      console.error('Error fetching embedded note:', error);
    }
  }
}
