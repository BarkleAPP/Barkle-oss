<template>
	<MkStickyContainer>
		...
		<MkSpacer :content-max="700">
			<div v-if="channel">
				...
				<!-- New section for pinned notes -->
				<div v-if="channel.pinnedNotes && channel.pinnedNotes.length > 0" class="_gap">
					<MkFolder :defaultOpen="true">
						<template #label>{{ i18n.ts.pinnedNotes }}</template>
						<XNote v-for="note in channel.pinnedNotes" :key="note.id" class="note _block" :note="note" :pinned="true"/>
					</MkFolder>
				</div>
	...
	const isOwnerOrAdmin = computed(() => {
		...
		return channel.value.userId === $i.id || 
			   channel.value.admins.some(admin => admin.id === $i.id);
	});
	
	const headerActions = computed(() => isOwnerOrAdmin.value ? [{
