<template>
	<div
		v-size="{ max: [310, 200] }" class="gafaadew"
	...
	>
		...
		<div class="form" :class="{ fixed }">
			...
			<footer>
				<!-- <button v-tooltip="i18n.ts.goLive" class="_button" href="/goLive"><i class="ph-video-camera-bold ph-lg"></i></button>-->
				<button v-tooltip="i18n.ts.attachFile" class="_button" @click="chooseFileFrom"><i class="ph-upload-bold ph-lg"></i></button>
				<button v-tooltip="i18n.ts.gif" class="_button" @click="pickGif"><i class="ph-gif-bold ph-lg"></i></button>
	...
	let imeText = $ref('');
	let postBarkleFor = $ref<boolean>(defaultStore.state.barkleForToggle);
	...
	*/
	...
	function addMissingMention() {
		...
		for (const x of extractMentions(ast)) {
			if (!visibleUsers.some(u => (u.username === x.username) && (u.host === x.host))) {
				os.api('users/show', { username: x.username }).then(user => {
  ...
  console.log('pickGif function called');
  os.openGifPicker(ev.currentTarget ?? ev.target, {}, (driveFile) => {
    console.log('GIF selected callback, driveFile:', driveFile);
    if (driveFile && driveFile.id) {
      // Ensure we're adding the file in a way that triggers reactivity
      files = [...files, driveFile];
      console.log('Files after push:', files);
    } else {
      console.log('No valid driveFile returned from GIF picker');
    }
  });
}	
	...
	async function onPaste(ev: ClipboardEvent) {
		...
		if (!props.renote && !quoteId && (paste.startsWith(url + '/notes/') || paste.startsWith(url + '/barks/'))) {
			ev.preventDefault();
  ...
  // Detect OS
  if (ua.includes('win')) {
    deviceType += ' on Windows';
  } else if (ua.includes('mac')) {
    deviceType += ' on macOS';
  } else if (ua.includes('linux')) {
    deviceType += ' on Linux';
  } else if (ua.includes('android')) {
    deviceType += ' on Android';
  ...
  return deviceType;
	...
	async function post() {
		let postData = {
			...
			poll: poll,
			barkleFor: postBarkleFor ? "Barkle for " + await detectDeviceType() : null,
