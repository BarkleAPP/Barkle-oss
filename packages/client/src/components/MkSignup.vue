...
let invitationCode: string = $ref(sessionStorage.getItem('pendingInviteCode') || '');
let email = $ref('');
...
function onSubmit(): void {
	...
	os.api('signup', {
		...
		password,
		emailAddress: email,
		invitationCode: invitationCode || pendingInviteCode,
		...
		if (instance.emailRequiredForSignup) {
			...
			os.api('signin', {
				...
				if (props.autoSet) {
					...
					// If there's a pending invite code, it was already processed during signup
					...
					// Redirect to root URL after signup
					setTimeout(() => {
						location.href = '/';
					}, 1000);
				}
