...
import XLive from '@/components/BkLive.vue';
...
// identify Apple Music URLs
if (requestUrl.hostname === 'music.apple.com') {
	const m = requestUrl.pathname.match(/\/(us|ca|uk|au|jp|de|fr)\/album\/.+\/(\d+)/);
	if (m) {
		isAppleMusic = true;
		appleMusicEmbedUrl = `https://embed.music.apple.com/${m[1]}/album/${m[2]}?app=music`;
	}
}

if (requestUrl.hostname === 'open.spotify.com') {
	const m = requestUrl.pathname.split('/');
	if (m[1] && m[2]) {
		spotifyType = m[1];
		spotifyId = m[2];
		isSpotify = true;
